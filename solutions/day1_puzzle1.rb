...
# worried when she called you down here. "At 500-year intervals into the past, someone
...
# 500 years further into the past every few days; that was the best we could do on such short notice."
#
...
# a sequence of changes in frequency (your puzzle input). A value like +6 means the 
...
CSV.foreach('input/day1.csv', headers: false) do |row|
  row_array = row[0].partition(/\+|-/)
  operator = row_array[1]
  number = row_array[2].to_i
  frequency = operator == '+' ? frequency + number : frequency - number
end

p "in the end the frequency is #{frequency}"
