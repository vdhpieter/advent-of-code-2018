...
# be finished before others can begin (your puzzle input). Each step is designated
...
26.times do
  ...
  next_step = free_steps.sort[0]
  result += next_step

  steps.delete(next_step)

  steps.each do |step, blockers|
    steps[step] = blockers - [next_step]
  end
end

p "The steps should be completed in this order: #{result}"
